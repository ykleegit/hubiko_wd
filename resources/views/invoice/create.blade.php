        ...
        $(document).on('change', "[name='invoice_type_radio']", function() {
            ...
            const handleVehicleOrMachineInvoice = () => {
                ...
                var label = `
                    ...
                    {{ Form::number('service_charge', null, ['class' => 'form-control service_charge', 'required' => 'required', 'id' => 'service_charge', 'placeholder' => 'Enter Service Charge', 'value' => '0']) }}
                `;
                $(".income_account_div").addClass('d-none');
                $(".account_id").removeAttr('required');
                $(".invoice_div").append(label);
            ...
            const handleRestaurantInvoice = () => {
                ...
                $(".tax_project_div").addClass('d-none');
                var label =
                    ` {{ Form::label('restaurant_order', __('Restaurant Orders'), ['class' => 'form-label']) }}<x-required></x-required>
                    {{ Form::select('restaurant_order', [], null, ['class' => 'form-control', 'required' => 'required']) }}`
                $(".invoice_div").append(label);
