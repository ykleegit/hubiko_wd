...
<head>
    ...
    <title>
        ...
        |
        {{ !empty(company_setting('title_text', $invoice->created_by, $invoice->workspace)) ? company_setting('title_text', $invoice->created_by, $invoice->workspace) : (!empty(admin_setting('title_text')) ? admin_setting('title_text') : 'Hubiko') }}
    </title>
    <link
                    ...
                    @if (isset($bank_details_list) && count($bank_details_list) > 0)
                        @foreach ($bank_details_list as $key => $bank)
                            <tr>
                                ...
                                <td>{{ !empty($bank->opening_balance) ? currency_format_with_sym($bank->opening_balance, $invoice->created_by, $invoice->workspace) : '' }}
                                </td>
