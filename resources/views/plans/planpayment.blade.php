                                ...
                                <div class="">
                                    ...
                                    <div class="cart-footer-total-row bg-primary text-white rounded p-3 d-flex align-items-center justify-content-between">
                                        <div class="mini-total-price">
                                            <div class="price">
                                                <h3 class="text-white mb-0 total">{{ super_currency_format_with_sym($planprice) }}</h3>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            $(".total").text(formatCurrency(planprice,'{{ $currency_setting }}'));
        });
                    ...
                    success: function(data)
                    {
                        if (data != '' ) {
                            if (data.is_success == true) {
                                    ...
                                    if ($('.switch-change').prop('checked')) {
                                    planprice = '{{ $planpriceyearly }}'; // Use yearly price if switch is ON
                                    }

                                    $('.total').html('<span class="original-price">' + formatCurrency(planprice,'{{ $currency_setting }}')+ '</span> / ' + '<span class="">'+finalPrice + '</span>');
