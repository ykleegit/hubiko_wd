    ...
    <script>
        $(document).on('change', "[name='proposal_type_radio']", function() {
            ...
            if (val == 'product') {
                ...
                var label =
                    ` {{ Form::label('project', __('Project'), ['class' => 'form-label']) }}<x-required></x-required> {{ Form::select('project', $projects, $proposal->category_id, ['class' => 'form-control', 'required' => 'required']) }}`
                $(".proposal_div").append(label);
...
@section('content')
    <div class="row">
        {{ Form::model($proposal, ['route' => ['proposal.update', $proposal->id], 'method' => 'PUT', 'class' => 'w-100', 'enctype' => 'multipart/form-data', 'class' => 'needs-validation', 'novalidate']) }}
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                            <div class="row" id="customer-box">
                                <div class="form-group col-md-6 " id="account-box">
                                    ...
                                    <input type="hidden" name="account_type" value="{{ $proposal->account_type ?? '' }}">
                                </div>
                                                ...
                                                <optgroup label="{{ $typeName }}">
                                                    @foreach ($subtypes as $subtypeId => $subtypeData)
                                                        ...
                                                        @foreach ($subtypeData['chart_of_accounts'] as $chartOfAccount)
                                                            <option value="{{ $chartOfAccount['id'] }}" {{ $chartOfAccount['id'] == $proposal->account_id ? 'selected' : ''}}>
                                                                &nbsp;&nbsp;&nbsp;{{ $chartOfAccount['account_name'] }}
                                                            </option>
                                                            @foreach ($subtypeData['subAccounts'] as $subAccount)
                                                                @if ($chartOfAccount['id'] == $subAccount['parent'])
                                                                <option value="{{ $subAccount['id'] }}" class="ms-5" {{ $subAccount['id'] == $proposal->account_id ? 'selected' : ''}}> &nbsp; &nbsp;&nbsp;&nbsp; {{' - '. $subAccount['account_name'] }}</option>
                                                                @endif
                                                            @endforeach
                                                        @endforeach
                                                    @endforeach
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            var valueToMatch = "{{ $proposal->account_type }}";
