        ...
        <div class="invoice-body">
            <table class="vertical-align-top">
                <tbody>
                    <tr>
                        <td>
                            <strong class="h5 mb-1">{{ __('Name ') }}:  </strong>{{ !empty($vendor->name) ? $vendor->name : '' }}</br>
                            <strong class="h5 mb-1">{{ __('Email ') }}:  </strong>{{ !empty($vendor->email) ? $vendor->email : '' }}
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <p class="mb-3">
                                ...
                                @else
                                    <strong class="h5 mb-1">{{ __('Name ') }}:
                                    </strong>{{ !empty($purchase->vender_name) ? $purchase->vender_name : '' }}
                                @endif
                            </p>
                        ...
                        @if ($settings['purchase_qr_display'] == 'on')
                            <td colspan="2">
                                <div class="view-qrcode" style="margin-top: 0;">
                                    <p> {!! DNS2D::getBarcodeHTML(route('purchases.link.copy',\Crypt::encrypt($purchase->purchase_id)), "QRCODE",2,2) !!}
                                </div>
                            </td>
                        @endif
                    </tr>
                        ...
                        <td>
                            <table class="no-space vertical-align-top" >
                                <tbody>
                                    <tr>
                                        <td>{{ __('Number: ')}}</td>
                                        <td class="text-right">{{\App\Models\Purchase::purchaseNumberFormat($purchase->purchase_id,$purchase->created_by,$purchase->workspace)}}</td>
                                    </tr>
                                    <tr>
                    ...
                    @if(isset($purchase->itemData) && count($purchase->itemData) > 0)
                        @foreach($purchase->itemData as $key => $item)
                            <tr>
                                ...
                                @endphp
                                <td>{{currency_format_with_sym($final_total,$purchase->created_by,$purchase->workspace)}}</td>
