...
use Hubiko\Taskly\Entities\Timesheet;
...
class Project extends Model
{
    ...
    {
        return \Hubiko\Taskly\Database\factories\ProjectFactory::new();
    }
    ...
    {
        return $this->hasMany('Hubiko\Taskly\Entities\Task', 'project_id', 'id');
    }
    ...
    {
        return $this->hasMany('Hubiko\Taskly\Entities\Milestone', 'project_id', 'id');
    }

    public function files()
    {
        return $this->hasMany('Hubiko\Taskly\Entities\ProjectFile', 'project_id', 'id');
    }

    public function activities()
    {
        return $this->hasMany('Hubiko\Taskly\Entities\ActivityLog', 'project_id', 'id')->orderBy('id', 'desc');
    }
    ...
    {
        $stage    = \Hubiko\Taskly\Entities\Stage::where('workspace_id', '=', getActiveWorkSpace())->where('complete', 1)->first();
        if (!empty($stage)) {
            $status = $stage->id;
        } else {
            $status = 0;
    ...
    public static function customerProject($customerId)
    {
        ...
        if (module_is_active('Account')) {
            $customer                   = \Hubiko\Account\Entities\Customer::find($customerId);
    ...
    public static function vendorProject($vendorId)
    {
        $project = '';
        if (module_is_active('Account')) {
            $vendor                   = \Hubiko\Account\Entities\Vender::find($vendorId);
