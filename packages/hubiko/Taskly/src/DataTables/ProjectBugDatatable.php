...
namespace Hubiko\Taskly\DataTables;
...
class ProjectBugDatatable extends DataTable
{
    ...
    public function query(BugReport $model): QueryBuilder
    {
        ...
        $bugs = $model->select('bug_reports.*' ,'bug_stages.name as stage_name','users.name as user_name')
        ->join('bug_stages','bug_stages.id', '=', 'bug_reports.status')
        ...
        if (\Auth::user() != 'client') {
            if (!Auth::user()->hasRole('client') && !Auth::user()->hasRole('company')) {
                $bugs->where('assign_to', '=', \Auth::user()->id);
            }
        }
       return $bugs;
    }
    ...
    public function getColumns(): array
    {
        $column = [
            ...
            Column::make('status')->title(__('Stage'))->name('bug_stages.name'),
