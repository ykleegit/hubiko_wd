...
namespace Hubiko\Taskly\Http\Controllers;
...
use Hubiko\Taskly\Entities\BugComment;
use Hubiko\Taskly\Entities\BugFile;
use Hubiko\Taskly\Entities\BugReport;
use Hubiko\Taskly\Entities\BugStage;
use Hubiko\Taskly\Entities\ClientProject;
use Hubiko\Taskly\Entities\Comment;
use Hubiko\Taskly\Entities\Milestone;
use Hubiko\Taskly\Entities\Project;
use Hubiko\Taskly\Entities\ProjectFile;
use Hubiko\Taskly\Entities\Stage;
use Hubiko\Taskly\Entities\SubTask;
use Hubiko\Taskly\Entities\Task;
use Hubiko\Taskly\Entities\TaskFile;
use Hubiko\Taskly\Entities\UserProject;
use Hubiko\Taskly\Entities\VenderProject;
use Hubiko\TimeTracker\Entities\TimeTracker;
use Hubiko\Taskly\Events\CreateBug;
use Hubiko\Taskly\Events\CreateMilestone;
use Hubiko\Taskly\Events\CreateProject;
use Hubiko\Taskly\Events\CreateTask;
use Hubiko\Taskly\Events\CreateTaskComment;
use Hubiko\Taskly\Events\DestroyBug;
use Hubiko\Taskly\Events\DestroyMilestone;
use Hubiko\Taskly\Events\DestroyProject;
use Hubiko\Taskly\Events\DestroyTask;
use Hubiko\Taskly\Events\DestroyTaskComment;
use Hubiko\Taskly\Events\UpdateBug;
use Hubiko\Taskly\Events\UpdateMilestone;
use Hubiko\Taskly\Events\UpdateProject;
use Hubiko\Taskly\Events\UpdateTask;
use Hubiko\Taskly\Events\UpdateTaskStage;
use Hubiko\Taskly\Events\ProjectInviteUser;
use Hubiko\Taskly\Events\ProjectShareToClient;
use Hubiko\Taskly\Events\ProjectUploadFiles;
use Hubiko\Taskly\Events\UpdateBugStage;
use Hubiko\Account\Entities\Bill;
use Hubiko\Documents\Entities\Document;
use Hubiko\Retainer\Entities\Retainer;
use App\Models\Proposal;
use App\Models\Purchase;
use Hubiko\Taskly\DataTables\ProjectBugDatatable;
use Hubiko\Taskly\DataTables\ProjectDatatable;
use Hubiko\Taskly\DataTables\ProjectTaskDatatable;
...
class ProjectController extends Controller
{
    ...
    public function create()
    {
        if (Auth::user()->isAbleTo('project create')) {
            if (module_is_active('CustomField')) {
                $customFields =  \Hubiko\CustomField\Entities\CustomField::where('workspace_id', getActiveWorkSpace())->where('module', '=', 'taskly')->where('sub_module', 'projects')->get();
            } else {
                $customFields = null;
            }
    ...
    public function store(Request $request)
    {
        ...
        if (Auth::user()->isAbleTo('project create')) {
            ...
            if (module_is_active('CustomField')) {
                \Hubiko\CustomField\Entities\CustomField::saveData($objProject, $request->customField);
            }
    ...
    public function edit(Project $project)
    {
        if (Auth::user()->isAbleTo('project edit')) {
            if (module_is_active('CustomField')) {
                $project->customField = \Hubiko\CustomField\Entities\CustomField::getData($project, 'taskly', 'projects');
                $customFields             = \Hubiko\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace())->where('module', '=', 'taskly')->where('sub_module', 'projects')->get();
            } else {
                $customFields = null;
            }
    ...
    public function update(Request $request, Project $project)
    {
        if (Auth::user()->isAbleTo('project edit')) {
            ...
            if (module_is_active('CustomField')) {
                \Hubiko\CustomField\Entities\CustomField::saveData($project, $request->customField);
            }
    ...
    public function destroy($projectID)
    {
        ...
        if ($project->created_by == $objUser->id || $objUser->type == 'company') {
            ...
            if ($task == 0 && $bug == 0) {
                ...
                if (module_is_active('CustomField')) {
                    $customFields = \Hubiko\CustomField\Entities\CustomField::where('module', 'taskly')->where('sub_module', 'projects')->get();
                    foreach ($customFields as $customField) {
                        $value = \Hubiko\CustomField\Entities\CustomFieldValue::where('record_id', '=', $projectID)->where('field_id', $customField->id)->first();
                        if (!empty($value)) {
                            $value->delete();
                        }
                    }
    ...
    public function taskCreate($projectID)
    {
        ...
        $currentWorkspace = getActiveWorkSpace();
        if (module_is_active('CustomField')) {
            $customFields =  \Hubiko\CustomField\Entities\CustomField::where('workspace_id', getActiveWorkSpace())->where('module', '=', 'taskly')->where('sub_module', 'tasks')->get();
        } else {
            $customFields = null;
        }
        if ($objUser->hasRole('client')) {
    ...
    public function taskStore(Request $request, $projectID)
    {
        ...
        if ($project) {
            ...
            if ($stage) {
                ...
                if (module_is_active('CustomField')) {
                    \Hubiko\CustomField\Entities\CustomField::saveData($task, $request->customField);
                }
    ...
    public function taskEdit($projectID, $taskId)
    {
        ...
        if (module_is_active('CustomField')) {
            $task->customField = \Hubiko\CustomField\Entities\CustomField::getData($task, 'taskly', 'tasks');
            $customFields             = \Hubiko\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace())->where('module', '=', 'taskly')->where('sub_module', 'tasks')->get();
        } else {
    ...
    public function taskDestroy($projectID, $taskID)
    {
        ...
        if (module_is_active('CustomField')) {
            $customFields = \Hubiko\CustomField\Entities\CustomField::where('module', 'taskly')->where('sub_module', 'tasks')->get();
            foreach ($customFields as $customField) {
                $value = \Hubiko\CustomField\Entities\CustomFieldValue::where('record_id', '=', $taskID)->where('field_id', $customField->id)->first();
                if (!empty($value)) {
                    $value->delete();
                }
            }
    ...
    public function bugReportCreate($project_id)
    {
        ...
        if (module_is_active('CustomField')) {
            $customFields =  \Hubiko\CustomField\Entities\CustomField::where('workspace_id', getActiveWorkSpace())->where('module', '=', 'taskly')->where('sub_module', 'bugs')->get();
        } else {
            $customFields = null;
        }
        if ($objUser->hasRole('client')) {
    ...
    public function bugReportStore(Request $request, $project_id)
    {
        ...
        if ($project) {
            ...
            if (module_is_active('CustomField')) {
                \Hubiko\CustomField\Entities\CustomField::saveData($bug, $request->customField);
            }
    ...
    public function bugReportEdit($project_id, $bug_id)
    {
        ...
        if (module_is_active('CustomField')) {
            $bug->customField = \Hubiko\CustomField\Entities\CustomField::getData($bug, 'taskly', 'bugs');
            $customFields             = \Hubiko\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace())->where('module', '=', 'taskly')->where('sub_module', 'bugs')->get();
        } else {
            $customFields = null;
        }
    ...
    public function bugReportDestroy($project_id, $bug_id)
    {
        ...
        if (module_is_active('CustomField')) {
            $customFields = \Hubiko\CustomField\Entities\CustomField::where('module', 'taskly')->where('sub_module', 'bugs')->get();
            foreach ($customFields as $customField) {
                $value = \Hubiko\CustomField\Entities\CustomFieldValue::where('record_id', '=', $bug_id)->where('field_id', $customField->id)->first();
                if (!empty($value)) {
                    $value->delete();
                }
            }
        ...
        return redirect()->back()->with('success', __('The bug has been deleted.'));
    }
