...
@section('page-action')
    <div class="d-flex">
        ...
        @permission('customer import')
            <a href="#" class="btn btn-sm btn-primary me-2" data-size="md" data-ajax-popup="true"
                data-title="{{ __('Customer Import') }}" data-url="{{ route('customer.file.import') }}" data-toggle="tooltip"
                title="{{ __('Import') }}"><i class="ti ti-file-import"></i>
            </a>
        @endpermission
        <a href="{{ route('customer.index') }}" class="btn btn-sm btn-primary me-2" data-bs-toggle="tooltip"
            title="{{ __('List View') }}">
            <i class="ti ti-list text-white"></i>
        </a>
        @permission('customer create')
            <a class="btn btn-sm btn-primary" data-ajax-popup="true" data-size="lg" data-title="{{ __('Create New Customer') }}"
                data-url="{{ route('customer.create') }}" data-bs-toggle="tooltip" data-bs-original-title="{{ __('Create') }}">
    ...
    <div class="row row-gap-2 mb-4">
                    ...
                    <div class="card-header border border-bottom h-100 p-3">
                        <div class="user-img-wrp d-flex align-items-center">
                            ...
                            <div class="user-content">
                                ...
                                @if ($customer->is_disable == 1 && !empty($customer['customer_id']))
                                        <a href="{{ route('customer.show', \Crypt::encrypt($customer['id'])) }}">
                                            <h4 class="mb-2 text-break">{{ !empty($customer->name) ? $customer->name : '' }}</h4>
                                        </a>
                                    @else
                                        <h4 class="mb-2 text-break">{{ !empty($customer->name) ? $customer->name : '' }}</h4>
                                    @endif
                                @else
                                    <h4 class="mb-2 text-break">{{ !empty($customer->name) ? $customer->name : '' }}</h4>
                                @endpermission
                            <span class="text-dark text-md">{{ $customer->email }}</span>
                        </div>
                        </div>
                    </div>
                        ...
                        <div class="bottom-icons d-flex flex-wrap align-items-center justify-content-between">
                            <div class="edit-btn-wrp d-flex flex-wrap align-items-center">
                                @if ($customer->is_disable == 1)
                                    @if (Laratrust::hasPermission('customer show') ||
                                        ...
                                        @if (!empty($customer['customer_id']))
                                            ...
                                            @permission('customer delete')
                                                ...
                                                <a href="#!" aria-label="Delete" data-confirm="{{ __('Are You Sure?') }}"
                                                    ...
                                                    data-bs-original-title="{{ __('Delete') }}" data-bs-toggle="tooltip"
                                                    data-bs-placement="top" class="btn btn-sm border show_confirm">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                        viewBox="0 0 16 16" fill="none">
                                ...
                                @else
                                    <a class="btn btn-sm border" data-title="{{ __('Lock') }}"
                                        data-bs-original-title="{{ __('Customer Is Disable') }}" data-bs-toggle="tooltip">
                                ...
                                <span class="badge bg-primary p-2 px-3">
                                    @permission('customer show')
                                        @if ($customer->is_disable == 1)
                                            <a href="{{ route('customer.show', \Crypt::encrypt($customer['id'])) }}"
                                                class="text-white">
                                                {{ Hubiko\Account\Entities\Customer::customerNumberFormat($customer['customer_id']) }}
                                            </a>
                                        @else
                                            <a class="text-white">
                                                {{ Hubiko\Account\Entities\Customer::customerNumberFormat($customer['customer_id']) }}
        ...
        <div class="col-xxl-3 col-xl-4 col-md-6">
            <a href="#" class="btn-addnew-project border-primary p-4" data-ajax-popup="true" data-size="lg"
                ...
                <h6 class="my-2 text-center">{{ __('New Customer') }}</h6>
