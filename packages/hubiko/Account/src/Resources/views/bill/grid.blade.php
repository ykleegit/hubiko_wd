                            ...
                            <div class="project-card">
                                <div class="project-card-inner">
                                    <div class="project-card-header d-flex justify-content-between h-100">
                                        ...
                                        @if ($bill->status == 0)
                                            <span
                                                class="badge bg-info p-2 px-3 bill_status">{{ __(Hubiko\Account\Entities\Bill::$statues[$bill->status]) }}</span>
                                        @elseif($bill->status == 1)
                                            <span
                                                class="badge bg-primary p-2 px-3 bill_status">{{ __(Hubiko\Account\Entities\Bill::$statues[$bill->status]) }}</span>
                                        @elseif($bill->status == 2)
                                            <span
                                                class="badge bg-secondary p-2 px-3 bill_status">{{ __(Hubiko\Account\Entities\Bill::$statues[$bill->status]) }}</span>
                                        @elseif($bill->status == 3)
                                            <span
                                                class="badge bg-warning p-2 px-3 bill_status">{{ __(Hubiko\Account\Entities\Bill::$statues[$bill->status]) }}</span>
                                        @elseif($bill->status == 4)
                                            <span
                                                class="badge bg-success p-2 px-3 bill_status">{{ __(Hubiko\Account\Entities\Bill::$statues[$bill->status]) }}</span>
                                        @endif
                                        <button type="button"
                                        ...
                                        <div class="project-content-top">
                                            <div class="user-info  d-flex align-items-center">
                                                @if (Laratrust::hasPermission('bill show'))
                                                    <a
                                                        href="{{ route('bill.show', \Crypt::encrypt($bill->id)) }}">{{ Hubiko\Account\Entities\Bill::billNumberFormat($bill->bill_id) }}</a>
                                                @else
                                                    <a>{{ Hubiko\Account\Entities\Bill::billNumberFormat($bill->bill_id) }}</a>
                                                @endif
                                            </div>
