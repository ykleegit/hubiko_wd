        ...
        if (module_is_active('Account')) {
            ...
            if ($invoice->status != 0) {
                ...
                foreach ($invoice_products as $invoice_product) {
                    $product       = \Hubiko\ProductService\Entities\ProductService::find($invoice_product->product_id);
                    $totalTaxPrice = 0;
                    if ($invoice->invoice_module == 'newspaper'){
                        $taxes = \Hubiko\Newspaper\Entities\NewspaperTax::tax($invoice_product->tax);
                        foreach ($taxes as $tax) {
                            if(!empty($tax)){
                                $taxPrice = \Hubiko\Newspaper\Entities\NewspaperTax::taxRate($tax->percentage, $invoice_product->price, $invoice_product->quantity);
                    ...
                    $data       = [
                        ...
                        'reference_sub_id'   => isset($invoice_product->id) ? $invoice_product->id : '',
                    ...
                    $data    = [
                        ...
                        'reference'          => 'Invoice',
                        'reference_id'       => $invoice->id,
                        'reference_sub_id'   => !empty($invoice_product)?$invoice_product->id:'',
                        'date'               => $invoice->issue_date,
                    ];
