...
namespace Hubiko\Account\Http\Controllers;
...
use Hubiko\Account\DataTables\BillDataTable;
use Hubiko\Account\Entities\AccountUtility;
use Hubiko\Account\Entities\BankAccount;
use Hubiko\Account\Entities\Bill;
use Hubiko\Account\Entities\BillAttechment;
use Hubiko\Account\Entities\BillPayment;
use Hubiko\Account\Entities\BillProduct;
use Hubiko\Account\Entities\ChartOfAccount;
use Hubiko\Account\Entities\ChartOfAccountSubType;
use Hubiko\Account\Entities\ChartOfAccountType;
use Hubiko\Account\Entities\CustomerDebitNotes;
use Hubiko\Account\Entities\StockReport;
use Hubiko\Account\Entities\Transaction;
use Hubiko\Account\Entities\Transfer;
use Hubiko\Account\Entities\Vender;
use Hubiko\Account\Events\CreateBill;
use Hubiko\Account\Events\CreatePaymentBill;
use Hubiko\Account\Events\DestroyBill;
use Hubiko\Account\Events\DuplicateBill;
use Hubiko\Account\Events\PaymentDestroyBill;
use Hubiko\Account\Events\ResentBill;
...
class BillController extends Controller
{
    ...
    public function create($vendorId)
    {
        if (module_is_active('ProductService')) {
            ...
            if (Auth::user()->isAbleTo('bill create')) {
                $category = \Hubiko\ProductService\Entities\Category::where('created_by', '=', creatorId())->where('workspace_id', getActiveWorkSpace())->where('type', 2)->get()->pluck('name', 'id');

                $bill_number = Bill::billNumberFormat($this->billNumber());

                $vendors = Vender::where('workspace', '=', getActiveWorkSpace())->get()->pluck('name', 'id');

                $product_services = \Hubiko\ProductService\Entities\ProductService::where('workspace_id', getActiveWorkSpace())->get()->pluck('name', 'id');
                ...
                if (module_is_active('CustomField')) {
                    $customFields = \Hubiko\CustomField\Entities\CustomField::where('workspace_id', getActiveWorkSpace())->where('module', '=', 'Account')->where('sub_module', 'Bill')->get();
                } else {
                    $customFields = null;
                }
                $product_type = \Hubiko\ProductService\Entities\ProductService::$product_type;
                if (module_is_active('Taskly')) {
                    if (module_is_active('ProductService')) {
                        $taxs = \Hubiko\ProductService\Entities\Tax::where('workspace_id', getActiveWorkSpace())->get()->pluck('name', 'id');

                    }
                    $projects = \Hubiko\Taskly\Entities\Project::select('projects.*')->join('user_projects', 'projects.id', '=', 'user_projects.project_id')->where('user_projects.user_id', '=', Auth::user()->id)->where('workspace', getActiveWorkSpace())->projectonly()->get()->pluck('name', 'id');
                }
    ...
    public function store(Request $request)
    {
        if (Auth::user()->isAbleTo('bill create')) {
            if ($request->bill_type == "product") {
                ...
                if (module_is_active('CustomField')) {
                    \Hubiko\CustomField\Entities\CustomField::saveData($bill, $request->customField);
                }
    ...
    public function show($e_id)
    {
        if (Auth::user()->isAbleTo('bill show')) {
            ...
            if (!empty($bill) && $bill->workspace == getActiveWorkSpace()) {
                ...
                if (module_is_active('CustomField')) {
                    $bill->customField = \Hubiko\CustomField\Entities\CustomField::getData($bill, 'Account', 'Bill');
                    $customFields = \Hubiko\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace())->where('module', '=', 'Account')->where('sub_module', 'Bill')->get();
                } else {
                    $customFields = null;
                }
        ...
        if (module_is_active('ProductService')) {
            if (Auth::user()->isAbleTo('bill edit')) {
                ...
                if (module_is_active('Taskly')) {
                    ...
                    $projects = \Hubiko\Taskly\Entities\Project::where('workspace', getActiveWorkSpace())->projectonly()->get()->pluck('name', 'id');
                }
                $product_type = \Hubiko\ProductService\Entities\ProductService::$product_type;
        ...
        if (Auth::user()->isAbleTo('bill delete'))
        {
            ...
            if(!$billDebitNotes)
            {
                if ($bill->workspace == getActiveWorkSpace()) {
                    ...
                    if (module_is_active('CustomField')) {
                        $customFields = \Hubiko\CustomField\Entities\CustomField::where('module', 'Account')->where('sub_module', 'Bill')->get();
                        foreach ($customFields as $customField) {
                            $value = \Hubiko\CustomField\Entities\CustomFieldValue::where('record_id', '=', $bill->id)->where('field_id', $customField->id)->first();
                            if (!empty($value)) {
                    ...
                    foreach ($billProducts as $key => $billProduct)
                    {
                        ...
                        $stocks = \Hubiko\Account\Entities\StockReport::where('type', '=', 'bill')->where('type_id', '=', $billProduct->invoice_id)->where('product_id',$billProduct->product_id)->get();
    ...
    {
        $data['product'] = $product = \Hubiko\ProductService\Entities\ProductService::find($request->product_id);
    ...
    public function payment($bill_id)
    {
        if (Auth::user()->isAbleTo('bill payment create')) {
            $bill = Bill::where('id', $bill_id)->first();
            $vendors = Vender::where('workspace', '=', getActiveWorkSpace())->get()->pluck('name', 'id');
            $categories = \Hubiko\ProductService\Entities\Category::where('created_by', '=', creatorId())->where('workspace_id', getActiveWorkSpace())->get()->pluck('name', 'id');
    ...
    public function bill($bill_id)
    {
        ...
        if (module_is_active('CustomField')) {
            $bill->customField = \Hubiko\CustomField\Entities\CustomField::getData($bill, 'Account', 'Bill');
            $customFields = \Hubiko\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace($bill->created_by))->where('module', '=', 'Account')->where('sub_module', 'Bill')->get();
        } else {
            $customFields = null;
        }
    ...
    public function productDestroy(Request $request)
    {
        if (Auth::user()->isAbleTo('bill payment delete')) {
            ...
            event(new \Hubiko\Account\Events\ProductDestroyBill($billProduct, $request));
    ...
    public function BillSectionGet(Request $request)
    {
        ...
        if ($request->type == "product") {
            ...
            $projects = \Hubiko\Taskly\Entities\Project::where('workspace', getActiveWorkSpace())->projectonly();
            ...
            $tasks = [];
            if (!empty($projects)) {
                $tasks = \Hubiko\Taskly\Entities\Task::where('project_id', $projects->id)->get()->pluck('title', 'id');
                if ($acction != 'edit') {
                    $tasks->prepend('--', '');
                }
            }
