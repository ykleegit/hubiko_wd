...
use Hubiko\Account\DataTables\VendorDataTable;
use Hubiko\Account\Events\CreateVendor;
use Hubiko\Account\Events\DestroyVendor;
use Hubiko\Account\Events\UpdateVendor;
...
class VenderController extends Controller
{
    ...
    public function create()
    {
        if (Auth::user()->isAbleTo('vendor create'))
        {
            if(module_is_active('CustomField')){
                $customFields =  \Hubiko\CustomField\Entities\CustomField::where('workspace_id',getActiveWorkSpace())->where('module', '=', 'Account')->where('sub_module','Vendor')->get();
            }else{
                $customFields = null;
            }
    ...
    public function store(Request $request)
    {
        if (Auth::user()->isAbleTo('vendor create'))
        {
            ...
            if(module_is_active('CustomField'))
            {
                \Hubiko\CustomField\Entities\CustomField::saveData($vendor, $request->customField);
            }
    ...
    public function show($e_id)
    {
        if (Auth::user()->isAbleTo('vendor show'))
        {
            ...
            if(module_is_active('CustomField')){
                $vendor->customField = \Hubiko\CustomField\Entities\CustomField::getData($vendor, 'Account','Vendor');
                $customFields             = \Hubiko\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace())->where('module', '=', 'Account')->where('sub_module','Vendor')->get();
            }else{
                $customFields = null;
            }
    ...
    public function destroy($id)
    {
        ...
        if (Auth::user()->isAbleTo('vendor delete'))
        {
            ...
            {
                if(module_is_active('CustomField')){
                    $customFields = \Hubiko\CustomField\Entities\CustomField::where('module','Account')->where('sub_module','Vendor')->get();
                    foreach($customFields as $customField)
                    {
                        $value = \Hubiko\CustomField\Entities\CustomFieldValue::where('record_id', '=', $vendor->id)->where('field_id',$customField->id)->first();
                        if(!empty($value)){
