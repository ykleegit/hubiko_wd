...
use Hubiko\Account\DataTables\BankAccountDataTable;
...
use Hubiko\Account\Events\CreateBankAccount;
use Hubiko\Account\Events\DestroyBankAccount;
use Hubiko\Account\Events\UpdateBankAccount;
...
class BankAccountController extends Controller
{
    ...
    public function invoicePayWithBankAccount(Request $request)
    {
        ...
        elseif($request->type == 'salesinvoice'){
            ...
            $invoice = \Hubiko\Sales\Entities\SalesInvoice::find('id',$request->invoice_id);
        }
    ...
    public function invoiceBankAccountRequestEdit($id)
    {
        ...
        if($bank_transfer_payment)
        {
            ...
            {
                $salesinvoice = \Hubiko\Sales\Entities\SalesInvoice::where('workspace',getActiveWorkSpace())->where('created_by',creatorId())->where('id',$bank_transfer_payment->request)->first();
                $invoice_id = \Hubiko\Sales\Entities\SalesInvoice::invoiceNumberFormat($salesinvoice->invoice_id);

            }
            elseif($bank_transfer_payment->type == 'retainer')
            {
                $retainer = \Hubiko\Retainer\Entities\Retainer::where('created_by',creatorId())->where('id',$bank_transfer_payment->request)->first();
                $invoice_id = \Hubiko\Retainer\Entities\Retainer::retainerNumberFormat($retainer->retainer_id);
            }
