...
use Hubiko\Account\Entities\Customer;
use Hubiko\Account\Entities\Payment;
use Hubiko\Account\Entities\Revenue;
use Hubiko\Account\Entities\StockReport;
use Hubiko\Account\Entities\Vender;
use Hubiko\Hrm\Entities\PaySlip;
use Hubiko\Training\Entities\Training;
...
class ReportController extends Controller
{
    ...
    public function incomeSummary(Request $request)
    {
        ...
        if (\Auth::user()->isAbleTo('report income manage')) {
            ...
            if (module_is_active('ProductService')) {
                $category = \Hubiko\ProductService\Entities\Category::where('created_by', '=', creatorId())->where('workspace_id', '=', getActiveWorkSpace())->where('type', '=', 1)->get()->pluck('name', 'id');
            }
            ...
            if (!empty($request->category)) {
                ...
                $cat = [];
                if (module_is_active('ProductService')) {
                    $cat = \Hubiko\ProductService\Entities\Category::find($request->category);
                }
            ...
            foreach ($tmpArray as $cat_id => $record) {
                ...
                $tmp['category'] = [];
                if (module_is_active('ProductService')) {
                    $tmp['category'] = !empty(\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->first())?\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->first()->name : '';
                }
                $tmp['data'] = [];
                for ($i = 1; $i <= 12; $i++) {
            ...
            foreach ($invoiceTmpArray as $cat_id => $record) {
                ...
                $invoice['category'] = [];
                if (module_is_active('ProductService')) {
                    $invoice['category'] = !empty(\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->where('type', '1')->first())?\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->where('type', '1')->first()->name : '';
                }
                $invoice['data'] = [];
                for ($i = 1; $i <= 12; $i++) {
    ...
    public function expenseSummary(Request $request)
    {
        ...
        if (Auth::user()->isAbleTo('report expense manage')) {
            ...
            foreach ($billTmpArray as $cat_id => $record) {
                ...
                $bill['category'] = [];
                if (module_is_active('ProductService')) {
                    $bill['category'] = !empty(\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->first())?\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->first()->name : '';
                }
                $bill['data'] = [];
                for ($i = 1; $i <= 12; $i++) {
            ...
            foreach ($purchaseTmpArray as $cat_id => $record) {
                ...
                $purchase['category'] = [];
                if (module_is_active('ProductService')) {
                    $purchase['category'] = !empty(\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->first())?\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->first()->name : '';
                }
                $purchase['data'] = [];
                for ($i = 1; $i <= 12; $i++) {
    ...
    public function incomeVsExpenseSummary(Request $request)
    {
        if (Auth::user()->isAbleTo('report income vs expense manage')) {
            ...
            $category = [];
            if (module_is_active('ProductService')) {
                $category = \Hubiko\ProductService\Entities\Category::where('workspace_id', '=', getActiveWorkSpace())->whereIn('type', [1, 2])->get()->pluck('name', 'id');
            }
            $data['monthList'] = $month = $this->yearMonth();
            $data['yearList'] = $this->yearList();
            ...
            foreach ($invoiceTmpArray as $cat_id => $record) {
                ...
                $invoice['category'] = [];
                if (module_is_active('ProductService')) {
                    $invoice['category'] = !empty(\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->first())?\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->first()->name : '';
                }
                $invoice['data'] = [];
                for ($i = 1; $i <= 12; $i++) {
    ...
    public function taxSummary(Request $request)
    {
        if (module_is_active('ProductService')) {
            if (Auth::user()->isAbleTo('report tax manage')) {
                $data['monthList'] = $month = $this->yearMonth();
                $data['yearList'] = $this->yearList();
                $data['taxList'] = $taxList = \Hubiko\ProductService\Entities\Tax::where('workspace_id', getActiveWorkSpace())->get();
    ...
    public function profitLossSummary(Request $request)
    {
        ...
        if (Auth::user()->isAbleTo('report loss & profit  manage')) {
            ...
            foreach ($invoiceTmpArray as $cat_id => $record) {
                ...
                $invoiceTmp['category'] = [];
                if (module_is_active('ProductService')) {
                    $invoiceTmp['category'] = !empty(\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->first())?\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->first()->name : '';
                }
                $invoiceSumData = [];
                for ($i = 1; $i <= 12; $i++) {
            ...
            foreach ($billTmpArray as $cat_id => $record) {
                ...
                $billTmp['category'] = [];
                if (module_is_active('ProductService')) {
                    $billTmp['category'] = !empty(\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->first())?\Hubiko\ProductService\Entities\Category::where('id', '=', $cat_id)->first()->name : '';
                }
                $billExpensSumData = [];
                for ($i = 1; $i <= 12; $i++) {
