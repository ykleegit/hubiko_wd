...
use Hubiko\Account\DataTables\RevenueDataTable;
...
class RevenueController extends Controller
{
    ...
    public function create()
    {
        if(Auth::user()->isAbleTo('revenue create'))
        {
            ...
            if(module_is_active('ProductService'))
            {
                $categories = \Hubiko\ProductService\Entities\Category::where('created_by', '=', creatorId())->where('workspace_id', getActiveWorkSpace())->where('type', 1)->get()->pluck('name', 'id');
            }
            else
            {
                $categories = [];
            ...
            $accounts = \Hubiko\Account\Entities\BankAccount::select(
                '*',
