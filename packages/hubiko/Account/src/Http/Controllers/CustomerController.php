...
use Hubiko\Account\DataTables\CustomerDataTable;
use Hubiko\Account\Events\CreateCustomer;
use Hubiko\Account\Events\DestroyCustomer;
use Hubiko\Account\Events\UpdateCustomer;
...
class CustomerController extends Controller
{
    ...
    public function create()
    {
        if(\Auth::user()->isAbleTo('customer create'))
        {
            if(module_is_active('CustomField')){
                $customFields =  \Hubiko\CustomField\Entities\CustomField::where('workspace_id',getActiveWorkSpace())->where('module', '=', 'Account')->where('sub_module','Customer')->get();
            }else{
                $customFields = null;
            }
    ...
    public function store(Request $request)
    {
        if (Auth::user()->isAbleTo('customer create'))
        {
            ...
            if(module_is_active('CustomField'))
            {
                \Hubiko\CustomField\Entities\CustomField::saveData($customer, $request->customField);
            }
    ...
    public function show(Request $request, $e_id)
    {
        if (Auth::user()->isAbleTo('customer show'))
        {
            ...
            if(module_is_active('CustomField')){
                $customer->customField = \Hubiko\CustomField\Entities\CustomField::getData($customer, 'Account','Customer');
                $customFields             = \Hubiko\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace())->where('module', '=', 'Account')->where('sub_module','Customer','customFields')->get();
            }else{
                $customFields = null;
            }
    ...
    public function destroy($id)
    {
        ...
        if (Auth::user()->isAbleTo('customer delete'))
        {
            ...
            if(module_is_active('CustomField')){
                $customFields = \Hubiko\CustomField\Entities\CustomField::where('module','Account')->where('sub_module','Customer')->get();
                foreach($customFields as $customField)
                {
                    $value = \Hubiko\CustomField\Entities\CustomFieldValue::where('record_id', '=', $customer->id)->where('field_id',$customField->id)->first();
                    if(!empty($value)){
                        $value->delete();
                    }
                }
