...
class CustomerDebitNotes extends Model
{
    ...
    {
        return $this->hasOne(\Hubiko\Account\Entities\Vender::class, 'id', 'vendor');
    }

    public function bill_number()
    {
        return $this->hasOne(\Hubiko\Account\Entities\Bill::class, 'id', 'bill');
    }
    ...
    public function usedDebitNote($debitNote)
    {
        if($debitNote->type == 'bill') {
            return $this->hasMany(\Hubiko\Account\Entities\DebitNote::class, 'debit_note', 'id')->sum('amount');
        }
        else {
