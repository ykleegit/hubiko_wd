...
<head>
    ...
    @if (isset($settings['body_fontfamily']))
        <style id="dynamic-fonts">
            {!! \Hubiko\LandingPage\Entities\LandingPageSetting::generateFontCSSFromJSON($settings['body_fontfamily']) !!}
        </style>
        <style>
            :root {
    ...
    @endif
    @php
        $pwa = \Hubiko\LandingPage\Entities\LandingPageSetting::pwa_store();
    @endphp
...
<body class="{{ !empty($themeColor)?$themeColor:'theme-1' }}" >
    ...
    @foreach (\Hubiko\LandingPage\Entities\Pixel::getPixelScript() as $script)
        <?= $script ?>
    @endforeach
</body>
