...
class MarketPlaceController extends Controller
{
    ...
    public function product_main_store(Request $request , $slug)
    {
        ...
        $settings = MarketplacePageSetting::where('module', $slug)->pluck('value', 'name')->toArray();

        $data['product_main_banner'] = $settings['product_main_banner'] ?? '';

        if($request->hasFile('product_main_banner')) {
