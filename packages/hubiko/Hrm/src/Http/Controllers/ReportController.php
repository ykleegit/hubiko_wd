...
use Hubiko\Hrm\Entities\Attendance;
use Hubiko\Hrm\Entities\Branch;
use Hubiko\Hrm\Entities\Department;
use Hubiko\Hrm\Entities\Employee;
use Hubiko\Hrm\Entities\Leave;
use Hubiko\Hrm\Entities\LeaveType;
    ...
    public function Payroll(Request $request)
    {
        if (Auth::user()->isAbleTo('payroll report manage')) {
                ...
                if (!in_array(Auth::user()->type, Auth::user()->not_emp_type)) {
                    ...
                    if (!empty($request->all())) {
                        $employees = Employee::select('employees.*', 'employees.name')
                            ...
                            ->where('pay_slips.salary_month', '<=', $request->end_month)->orderBy('employees.id', 'asc');
                    } else {
                        $employees = Employee::where('workspace', getActiveWorkSpace());
                    }
            ...
            if (!empty($request->all()) && $request->is_export == 'yes') {
                if (empty($request->report_type) || empty($request->employees)) {
                    return redirect()->back()->with('error', __('Please select report type and employees.'));
                }
                // For Final Total
                $final_total = [];
                $final_total[] = 'Total';
