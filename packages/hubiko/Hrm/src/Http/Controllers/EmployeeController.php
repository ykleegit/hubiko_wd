...
namespace Hubiko\Hrm\Http\Controllers;
...
use Hubiko\Hrm\Entities\Branch;
use Hubiko\Hrm\Entities\Department;
use Hubiko\Hrm\Entities\Designation;
use Hubiko\Hrm\Entities\DocumentType;
use Hubiko\Hrm\Entities\Employee;
use Hubiko\Hrm\Entities\EmployeeDocument;
...
use Hubiko\Hrm\Entities\Termination;
use Hubiko\Hrm\Events\CreateEmployee;
use Hubiko\Hrm\Events\DestroyEmployee;
use Hubiko\Hrm\Events\UpdateEmployee;
use Illuminate\Validation\Rule;
use Hubiko\Hrm\DataTables\EmployeeDataTable;
...
class EmployeeController extends Controller
{
    ...
    public function create()
    {
        if (Auth::user()->isAbleTo('employee create')) {
            ...
            if (module_is_active('CustomField')) {
                $customFields =  \Hubiko\CustomField\Entities\CustomField::where('workspace_id', getActiveWorkSpace())->where('module', '=', 'hrm')->where('sub_module', 'Employee')->get();
            } else {
                $customFields = null;
            }
    ...
    public function store(Request $request)
    {
        ...
        if (Auth::user()->isAbleTo('employee create')) {
            ...
            if (module_is_active('CustomField')) {
                \Hubiko\CustomField\Entities\CustomField::saveData($employee, $request->customField);
            }
    ...
    public function show($id)
    {
        if (Auth::user()->isAbleTo('employee show')) {
            ...
            if (!empty($employee)) {
                ...
                $location_type    = Employee::$location_type;
                if (module_is_active('CustomField')) {
                    $employee->customField = \Hubiko\CustomField\Entities\CustomField::getData($employee, 'hrm', 'Employee');
                    $customFields             = \Hubiko\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace())->where('module', '=', 'hrm')->where('sub_module', 'Employee')->get();
                } else {
                    $customFields = null;
                }
    ...
    public function destroy($id)
    {
        if (Auth::user()->isAbleTo('employee delete')) {
            ...
            if (!empty($employee)) {
                ...
                if (module_is_active('CustomField')) {
                    $customFields = \Hubiko\CustomField\Entities\CustomField::where('module', 'Hrm')->where('sub_module', 'Employee')->get();
                    foreach ($customFields as $customField) {
                        $value = \Hubiko\CustomField\Entities\CustomFieldValue::where('record_id', '=', $employee->id)->where('field_id', $customField->id)->first();
                        if (!empty($value)) {
                            $value->delete();
                        }
                    }
