                    ...
                    <table class="table" id="">
                        ...
                        <tbody>
                            @forelse ($employees as $employee)
                                @php
                                    $attendance = Hubiko\Hrm\Entities\Employee::present_status($employee->id, isset($_GET['date']) ? $_GET['date'] : date('Y-m-d'));
                                @endphp
                                <tr>
                                    <td class="Id">
                                        ...
                                        @if (!empty($employee->employee_id))
                                            @permission('employee show')
                                                <a class="btn btn-outline-primary"
                                                    href="{{ route('employee.show', \Illuminate\Support\Facades\Crypt::encrypt($employee->id)) }}">{{ Hubiko\Hrm\Entities\Employee::employeeIdFormat($employee->employee_id) }}</a>
                                            @else
                                                <a
                                                    class="btn btn-outline-primary">{{ Hubiko\Hrm\Entities\Employee::employeeIdFormat($employee->employee_id) }}</a>
                                            @endpermission
                                        @endif
                                    </td>
                                    <td>{{ $employee->name }}</td>
                                    <td>
                                        {{ !empty(Hubiko\Hrm\Entities\Employee::Branchs($employee->branch_id)) ? Hubiko\Hrm\Entities\Employee::Branchs($employee->branch_id)->name : '--' }}
                                    </td>
                                    <td>
                                        {{ !empty(Hubiko\Hrm\Entities\Employee::Departments($employee->department_id)) ? Hubiko\Hrm\Entities\Employee::Departments($employee->department_id)->name : '--' }}
                                    </td>
                                    <td>
                                        <div class="row">
                                            <div class="col-md-1">
