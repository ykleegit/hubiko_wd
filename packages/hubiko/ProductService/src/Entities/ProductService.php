...
use Illuminate\Database\Eloquent\Factories\HasFactory;
    ...
    {
        return \Hubiko\ProductService\Database\factories\ProductServiceFactory::new();
    }

    public function units()
    {
        return $this->belongsTo('Hubiko\ProductService\Entities\Unit', 'unit_id');
    }

    public function categorys()
    {
        return $this->belongsTo('Hubiko\ProductService\Entities\Category', 'category_id');
    }
    public function taxes()
    {
        return $this->hasOne('Hubiko\ProductService\Entities\Tax', 'id', 'tax_id')->first();
    }

    public function unit()
    {
        return $this->hasOne('Hubiko\ProductService\Entities\Unit', 'id', 'unit_id')->first();
    }

    public function category()
    {
        return $this->hasOne('Hubiko\ProductService\Entities\Category', 'id', 'category_id');
    }
    ...
    public function getTotalProductQuantity()
    {
        if(module_is_active('Pos'))
        {
            ...
            $poses = \Hubiko\Pos\Entities\Pos::where('created_by', creatorId())->where('workspace',getActiveWorkSpace());
            if ($this->isUser())
            {
            ...
            {
                $positem = \Hubiko\Pos\Entities\PosProduct::select('quantity')->where('pos_id', $pos->id)->where('product_id', $product_id)->first();
    ...
    {
        $stocks                 = new \Hubiko\Account\Entities\StockReport();
    ...
    public function getProductQuantity()
    {
        if(module_is_active('Pos'))
        {
            ...
            $quotations = \Hubiko\Quotation\Entities\Quotation::where('created_by', creatorId())->where('workspace',getActiveWorkSpace());
            ...
            {
                $positem = \Hubiko\Quotation\Entities\QuotationProduct::select('quantity')->where('quotation_id', $quotation->id)->where('product_id', $product_id)->first();
