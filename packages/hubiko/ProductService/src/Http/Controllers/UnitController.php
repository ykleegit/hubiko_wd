...
use Hubiko\ProductService\Events\CreateUnit;
use Hubiko\ProductService\Events\DestroyUnit;
use Hubiko\ProductService\Events\UpdateUnit;
...
class UnitController extends Controller
{
    ...
    public function store(Request $request)
    {
        if(Auth::user()->isAbleTo('unit cerate'))
        {
            ...
            return redirect()->route('category.index')->with('success', __('The unit has been created successfully.'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function update(Request $request, $id)
    {
        if(Auth::user()->isAbleTo('unit edit'))
        {
            ...
            return redirect()->route('category.index')->with('success', __('The unit details are updated successfully.'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function destroy($id)
    {
        if(Auth::user()->isAbleTo('unit delete'))
        {
            ...
            $unit->delete();
            return redirect()->route('category.index')->with('success', __('The unit has been deleted.'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
