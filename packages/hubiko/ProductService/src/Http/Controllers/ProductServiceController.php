...
namespace Hubiko\ProductService\Http\Controllers;
...
use Hubiko\ProductService\Entities\Category;
use Hubiko\ProductService\Entities\ProductService;
use Hubiko\ProductService\Entities\Tax;
use Hubiko\ProductService\Entities\Unit;
...
use Hubiko\ProductService\Entities\ProductsLogTime;
use Illuminate\Support\Facades\DB;
use Hubiko\ProductService\Events\CreateProduct;
use Hubiko\ProductService\Events\DestroyProduct;
use Hubiko\ProductService\Events\UpdateProduct;
use Hubiko\CMMS\Entities\Workorder;
use Hubiko\CMMS\Entities\Component;
use Hubiko\CMMS\Entities\Pms;
use Hubiko\Bookings\Entities\BookingsDuration;
use Hubiko\ConsignmentManagement\Entities\ConsignmentWeight;
use Hubiko\RestaurantMenu\Entities\RestaurantSystemSet;
use Hubiko\RestaurantMenu\Entities\RestaurantVarient;
use Hubiko\Facilities\Entities\FacilitiesSpace;
use Hubiko\Facilities\Entities\FacilitiesService;
use Hubiko\JewelleryStoreManagement\Entities\JewelleryItem;
use Hubiko\ProductService\DataTables\ProductServiceDataTable;
use Hubiko\OpticalAndEyeCareCenter\Entities\OpticalEyeCareItems;
...
class ProductServiceController extends Controller
{
    ...
    public function index(ProductServiceDataTable $dataTable)
    {
        if (Auth::user()->isAbleTo('product&service manage')) {
            ...
            if (module_is_active(('HairAndCareStudio'))) {
                $product_type['hair & care studio'] = 'Hair & Care Studio';
            }
    ...
    public function ProductSectionGet(Request $request)
    {
        ...
        if (module_is_active('CustomField')) {
            $customFields =  \Hubiko\CustomField\Entities\CustomField::where('workspace_id', getActiveWorkSpace())->where('module', '=', 'ProductService')->where('sub_module', 'product & service')->get();
        } else {
            $customFields = null;
        }
        ...
        } elseif ($type == 'service' || $type == 'rent' || $type == 'music institute' || $type == 'fleet' || $type == 'hair & care studio') {
    ...
    public function store(Request $request)
    {
        if (Auth::user()->isAbleTo('product&service create')) {
            ...
            if (module_is_active('CustomField')) {
                \Hubiko\CustomField\Entities\CustomField::saveData($productService, $request->customField);
            }
    ...
    public function GetItem(Request $request)
    {
        if ($request->type == 'edit') {
            $product_services = \Hubiko\ProductService\Entities\ProductService::where('workspace_id', getActiveWorkSpace());
        } else {
            $product_services = \Hubiko\ProductService\Entities\ProductService::where('workspace_id', getActiveWorkSpace())->whereNotIn('id', explode(',', $request->productItems));
        }
    ...
    public function getTaxes(Request $request)
    {
        if (module_is_active('ProductService')) {
            $taxs_data = \Hubiko\ProductService\Entities\Tax::whereIn('id', $request->tax_id)->where('workspace_id', getActiveWorkSpace())->select('id', 'name', 'rate')->get();
