...
<head>
    ...
    <title>{{ \WorkDo\Pos\Entities\Pos::posNumberFormat($pos->pos_id)}} | {{ !empty(company_setting('title_text')) ? company_setting('title_text') : (!empty(admin_setting('title_text')) ? admin_setting('title_text') :'Hubiko') }}</title>
    <link
    ...
    <div class="invoice-preview-main" id="boxes">
        <div class="invoice-header">
            <table class="vertical-align-top">
                <tbody>
                    <tr>
                        <td >
                            <h3 style=" display: block; text-transform: uppercase; font-size: 30px; font-weight: bold; padding: 15px; background: var(--theme-color); color: {{$font_color}};">{{ __('POS')}}</h3>
                            ...
                            <table class="no-space">
                                <tbody>
                                    <tr>
                                        <td>{{ __('Number: ')}}</td>
                                        <td class="text-right">{{\WorkDo\Pos\Entities\Pos::posNumberFormat($pos->pos_id)}}</td>
                                    </tr>
                                    <tr>
                                        <td>{{ __('POS Date:')}}</td>
        ...
        <div class="invoice-body">
            ...
            <div class="invoice-footer">
                <p> {{$settings['pos_footer_title']}} <br>
                    {{$settings['pos_footer_notes']}} </p>
            </div>
        </div>
    </div>
    @if(!isset($preview))
