...
namespace Hubiko\Pos\Http\Controllers;
...
use Hubiko\Pos\Entities\Pos;
use App\Models\WarehouseProduct;
use Hubiko\Pos\Entities\PosProduct;
use Hubiko\Pos\Entities\PosPayment;
use Hubiko\Pos\Entities\PosUtility;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Crypt;
use App\Models\User;
use App\Models\Setting;
use App\Models\WorkSpace;
use Hubiko\Pos\DataTables\PosOrderDataTable;
use DB;
use Hubiko\Pos\Events\CreatePaymentPos;
use Stripe\Product;
use Hubiko\Pos\DataTables\BarcodeDataTable;
use Hubiko\Quotation\Entities\Quotation;
use Hubiko\Quotation\Entities\QuotationProduct;
...
class PosController extends Controller
{
    ...
    public function index(Request $request)
    {
        if (\Auth::user()->isAbleTo('pos add manage'))
        {
            ...
            if (isset($cart) && count($cart) > 0)
            {
                ...
                {
                    $product = \Hubiko\ProductService\Entities\ProductService::where('created_by', creatorId())->where('workspace_id',getActiveWorkSpace())->whereIn('id',array_keys($cart))->get();
                    if(count($product) == 0)
                    {
                        session()->forget('pos');
                    }
    ...
    public function create(Request $request)
    {
        $sess = session()->get('pos');
            if (Auth::user()->isAbleTo('pos add manage') && isset($sess) && !empty($sess) && count($sess) > 0) {
            $user = Auth::user();
            ...
            if(module_is_active('Account'))
            {
                ...
                if(!empty($user->id))
                {
                    ...
                    $customer = \Hubiko\Account\Entities\Customer::where('user_id',$user->id)->where('name', '=', $request->vc_name)->where('created_by', creatorId())->where('workspace',getActiveWorkSpace())->first();
                    if(!empty($customer)){
                        $customer = $customer;
                    }
                    else{
            ...
            if (isset($sales) && !empty($sales) && count($sales) > 0) {
                ...
                if (count($result) > 0) {
                    ...
                    foreach ($sales as $key => $value) {
                        $product_id = $value['id'];
                        $product = \Hubiko\ProductService\Entities\ProductService::whereId($product_id)->where('created_by', $user_id)->where('workspace_id',getActiveWorkSpace())->first();
                        $original_quantity = ($product == null) ? 0 : (int)$product->quantity;
                        $product_quantity = $original_quantity - $value['quantity'];
                        if ($product != null && !empty($product)) {
                            \Hubiko\ProductService\Entities\ProductService::where('id', $product_id)->update(['quantity' => $product_quantity]);
                        }
                        ...
                        if(module_is_active('Account'))
                        {
                            $type='Pos';
                            $type_id = $pos->id;
                            \Hubiko\Account\Entities\StockReport::where('type','=','pos')->where('type_id' ,'=', $pos->id)->delete();
    ...
    {
        if(\Auth::user()->isAbleTo('pos order manage'))
        {
    ...
    public function addToCart(Request $request, $id,$session_key,$war)
    {
        if (Auth::user()->isAbleTo('pos cart manage') && $request->ajax()) {
            $product = \Hubiko\ProductService\Entities\ProductService::find($id);
            ...
            $taxes=\Hubiko\Pos\Entities\Pos::tax($product->tax_id);
            $totalTaxRate=\Hubiko\Pos\Entities\Pos::totalTaxRate($product->tax_id);
            $product_tax='';
            $product_tax_id=[];
    ...
    {
        if(\Auth::user()->isAbleTo('pos setting manage'))
        {
    ...
    public function dashboard(Request $request)
    {
        if (\Auth::user()->isAbleTo('pos dashboard manage'))
        {
            ...
            if(module_is_active('Account'))
            {
                $customers = \Hubiko\Account\Entities\Customer::select('id')->where('created_by', creatorId())->where('workspace',getActiveWorkSpace())->count();

                $vendors = \Hubiko\Account\Entities\Vender::select('id')->where('created_by', creatorId())->where('workspace',getActiveWorkSpace())->count();
            }
    ...
    {
        if (Auth::user()->isAbleTo('print_barcode manage')) {
            if(module_is_active('ProductService'))
            {
                $barcode = [
    ...
    public function getproduct(Request $request)
    {
        if ($request->warehouse_id == 0) {
            ...
            if(module_is_active('ProductService'))
            {
                $productServices = \Hubiko\ProductService\Entities\ProductService::whereIn('id', $productServicesId)->where('created_by', creatorId())->where('workspace_id',getActiveWorkSpace())->get()->pluck('name', 'id')->toArray();
            }else{
                $productServices = [];
            }
        }
    ...
    public function receipt(Request $request)
    {
        if (Auth::user()->isAbleTo('print_barcode manage')) {
            if (!empty($request->product_id)) {
                if(module_is_active('ProductService'))
                {
                    $productServices = \Hubiko\ProductService\Entities\ProductService::whereIn('id', $request->product_id)->where('created_by', creatorId())->where('workspace_id',getActiveWorkSpace())->get();
                }else{
                    $productServices = [];
                }
                $quantity = $request->quantity;
