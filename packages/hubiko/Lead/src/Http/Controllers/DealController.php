...
use Hubiko\Lead\Entities\ClientPermission;
use Hubiko\Lead\Entities\Deal;
use Hubiko\Lead\Entities\DealActivityLog;
...
use Hubiko\Lead\DataTables\DealDataTable;
use Hubiko\Lead\Events\CreateDeal;
use Hubiko\Lead\Events\CreateDealTask;
use Hubiko\Lead\Events\DealAddCall;
use Hubiko\Lead\Events\DealAddClient;
use Hubiko\Lead\Events\DealAddDiscussion;
use Hubiko\Lead\Events\DealAddEmail;
use Hubiko\Lead\Events\DealAddNote;
use Hubiko\Lead\Events\DealAddProduct;
use Hubiko\Lead\Events\DealMoved;
use Hubiko\Lead\Events\StatusChangeDealTask;
use Hubiko\Lead\Events\UpdateDealTask;
use Hubiko\Lead\Events\DestroyDeal;
use Hubiko\Lead\Events\DestroyDealTask;
use Hubiko\Lead\Events\DestroyUserDeal;
use Hubiko\Lead\Events\UpdateDeal;
use Hubiko\Lead\Events\DealAddUser;
use Hubiko\Lead\Events\DealCallUpdate;
use Hubiko\Lead\Events\DealSourceUpdate;
use Hubiko\Lead\Events\DealUploadFile;
use Hubiko\Lead\Events\DestroyDealCall;
use Hubiko\Lead\Events\DestroyDealClient;
use Hubiko\Lead\Events\DestroyDealfile;
use Hubiko\Lead\Events\DestroyDealProduct;
use Hubiko\Lead\Events\DestroyDealSource;
...
class DealController extends Controller
{
    ...
    public function create()
    {
        if (Auth::user()->isAbleTo('deal create')) {
            ...
            if (module_is_active('CustomField')) {
                $customFields =  \Hubiko\CustomField\Entities\CustomField::where('workspace_id', $getActiveWorkSpace)->where('module', '=', 'lead')->where('sub_module', 'deal')->get();
            } else {
                $customFields = null;
            }
    ...
    public function store(Request $request)
    {
        ...
        if ($usr->isAbleTo('deal create')) {
            ...
            if (empty($stage)) {
                ...
                if (module_is_active('CustomField')) {
                    \Hubiko\CustomField\Entities\CustomField::saveData($deal, $request->customField);
                }
    ...
    public function show(Deal $deal)
    {
        if (Auth::user()->isAbleTo('deal show')) {
            if ($deal->is_active) {
                ...
                if (module_is_active('CustomField')) {
                    $deal->customField = \Hubiko\CustomField\Entities\CustomField::getData($deal, 'lead', 'deal');
                    $customFields      = \Hubiko\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace())->where('module', '=', 'lead')->where('sub_module', 'deal')->get();
                } else {
                    $customFields = null;
                }
    ...
    public function destroy(Deal $deal)
    {
        if (Auth::user()->isAbleTo('deal delete')) {
            ...
            if ($deal->created_by == $creatorId && $deal->workspace_id == $getActiveWorkSpace) {
                ...
                if (module_is_active('CustomField')) {
                    $customFields = \Hubiko\CustomField\Entities\CustomField::where('module', 'lead')->where('sub_module', 'deal')->get();
                    foreach ($customFields as $customField) {
                        $value = \Hubiko\CustomField\Entities\CustomFieldValue::where('record_id', '=', $deal->id)->where('field_id', $customField->id)->first();
                        if (!empty($value)) {
                            $value->delete();
                        }
                    }
    ...
    public function productEdit($id)
    {
        if (Auth::user()->isAbleTo('deal edit')) {
            ...
            $deal = Deal::find($id);
            if ($deal->created_by == $creatorId && $deal->workspace_id == $getActiveWorkSpace) {
                $products = \Hubiko\ProductService\Entities\ProductService::where('workspace_id', '=', $getActiveWorkSpace)->where('created_by', '=', $creatorId)->whereNOTIn('id', explode(',', $deal->products))->get()->pluck('name', 'id');
