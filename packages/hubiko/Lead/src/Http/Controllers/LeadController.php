...
use Hubiko\Lead\Entities\DealCall;
use Hubiko\Lead\Entities\DealDiscussion;
use Hubiko\Lead\Entities\DealEmail;
use Hubiko\Lead\Entities\DealFile;
use Hubiko\Lead\Entities\DealStage;
use Hubiko\Lead\Entities\DealTask;
use Hubiko\Lead\Entities\Label;
use Hubiko\Lead\Entities\Lead;
use Hubiko\Lead\Entities\LeadActivityLog;
use Hubiko\Lead\Entities\LeadCall;
use Hubiko\Lead\Entities\LeadDiscussion;
use Hubiko\Lead\Entities\LeadEmail;
use Hubiko\Lead\Entities\LeadFile;
use Hubiko\Lead\Entities\LeadStage;
use Hubiko\Lead\Entities\Pipeline;
use Hubiko\Lead\Entities\Source;
use Hubiko\Lead\Entities\User as EntitiesUser;
use Hubiko\Lead\Entities\UserDeal;
use Hubiko\Lead\Entities\UserLead;
use Hubiko\Lead\Entities\LeadUtility;
use Hubiko\ProductService\Entities\ProductService;
use App\Models\Role;
use Illuminate\Support\Facades\Mail;
use Hubiko\Lead\DataTables\LeadDataTable;
use Hubiko\Lead\Entities\LeadTask;
use Hubiko\Lead\Events\CreateLead;
use Hubiko\Lead\Events\CreateLeadTask;
use Hubiko\Lead\Events\DestroyLead;
use Hubiko\Lead\Events\DestroyLeadCall;
use Hubiko\Lead\Events\DestroyLeadFile;
use Hubiko\Lead\Events\DestroyLeadProduct;
use Hubiko\Lead\Events\DestroyLeadSource;
use Hubiko\Lead\Events\DestroyLeadTask;
use Hubiko\Lead\Events\DestroyLeadUser;
use Hubiko\Lead\Events\LeadAddCall;
use Hubiko\Lead\Events\LeadAddDiscussion;
use Hubiko\Lead\Events\LeadAddEmail;
use Hubiko\Lead\Events\LeadAddNote;
use Hubiko\Lead\Events\LeadAddProduct;
use Hubiko\Lead\Events\LeadAddUser;
use Hubiko\Lead\Events\LeadConvertDeal;
use Hubiko\Lead\Events\LeadMoved;
use Hubiko\Lead\Events\LeadSourceUpdate;
use Hubiko\Lead\Events\LeadUpdateCall;
use Hubiko\Lead\Events\LeadUploadFile;
use Hubiko\Lead\Events\StatusChangeLeadTask;
use Hubiko\Lead\Events\UpdateLead;
use Hubiko\Lead\Events\UpdateLeadTask;
...
class LeadController extends Controller
{
    ...
    public function create()
    {
        if (Auth::user()->isAbleTo('lead create')) {
            ...
            if (module_is_active('CustomField')) {
                $customFields =  \Hubiko\CustomField\Entities\CustomField::where('workspace_id', $getActiveWorkSpace)->where('module', '=', 'lead')->where('sub_module', 'lead')->get();
            } else {
                $customFields = null;
            }
    ...
    public function store(Request $request)
    {
        ...
        if ($usr->isAbleTo('lead create')) {
            ...
            if (empty($stage)) {
                ...
                if (module_is_active('CustomField')) {
                    \Hubiko\CustomField\Entities\CustomField::saveData($lead, $request->customField);
                }
    ...
    public function show(Lead $lead)
    {
        if (Auth::user()->isAbleTo('lead show')) {
            if ($lead->is_active) {
                ...
                if (module_is_active('CustomField')) {
                    $lead->customField = \Hubiko\CustomField\Entities\CustomField::getData($lead, 'lead', 'lead');
                    $customFields      = \Hubiko\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace())->where('module', '=', 'lead')->where('sub_module', 'lead')->get();
                } else {
                    $customFields = null;
                }
    ...
    public function destroy(Lead $lead)
    {
        if (Auth::user()->isAbleTo('lead delete')) {
            ...
            if (module_is_active('CustomField')) {
                $customFields = \Hubiko\CustomField\Entities\CustomField::where('module', 'lead')->where('sub_module', 'lead')->get();
                foreach ($customFields as $customField) {
                    $value = \Hubiko\CustomField\Entities\CustomFieldValue::where('record_id', '=', $lead->id)->where('field_id', $customField->id)->first();
                    if (!empty($value)) {
                        $value->delete();
                    }
                }
    ...
    public function  productEdit($id)
    {
        if (Auth::user()->isAbleTo('lead edit')) {
            ...
            if ($lead->created_by == creatorId()) {
                $products = [];
                if (module_is_active('ProductService')) {
                    $products = \Hubiko\ProductService\Entities\ProductService::where('created_by', '=', $creatorId)->where('workspace_id', $getActiveWorkSpace)->whereNOTIn('id', explode(',', $lead->products))->where('workspace_id', $getActiveWorkSpace)->get()->pluck('name', 'id');
                }
