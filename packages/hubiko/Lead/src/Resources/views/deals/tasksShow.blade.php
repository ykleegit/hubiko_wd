    ...
    <div class="row text-sm">
        <div class="col-md-4">
            <div class="mb-4">
                <b>{{__('Status')}}</b>
                @if($task->status)
                    <div class="badge p-2 px-3 bg-success mb-1">{{__(Hubiko\Lead\Entities\DealTask::$status[$task->status])}}</div>
                @else
                    <div class="badge p-2 px-3 bg-warning mb-1">{{__(Hubiko\Lead\Entities\DealTask::$status[$task->status])}}</div>
                @endif
            </div>
        </div>
        <div class="col-md-4">
            <div class="mb-4">
                <b>{{__('Priority')}}</b>
                <p>{{__(Hubiko\Lead\Entities\DealTask::$priorities[$task->priority])}}</p>
            </div>
        </div>
        <div class="col-md-4">
            <div class="mb-4">
